      ...
      rows.each do |row|
        ...
        row.css('td').each do |field|
          ...
          if race_fields_array[index] == " Sex/Age"
            ...
            if !field.text.empty?
              if first_field
                result.update_attributes("residence" => field.text.encode('UTF-8'))
                first_field = false
              else
      ...
      rows.each do |row|
        ...
        row.css('td').each do |field|
          ...
          if race_fields_array[index] == "Age"
            if !field.text.empty?
              result.update_attributes("sex" => field.text[2].downcase)
              result.update_attributes("age" => field.text[0..1].to_i)
            end
